<script>
	import { isEvilStore, searchQueryStore, activeTopicStore } from "$lib/theme";
	...
	let displayBets = $derived(
		bets
			.filter((b) => {
				...
				if (topic === "Trending" || topic === "New" || topic === "Expiring Soon")
					return true;
			...
			.sort((a, b) => {
				...
				if ($activeTopicStore === "New") {
					const getH = (s) => (s && s.includes("day") ? parseInt(s) * 24 : parseInt(s));
