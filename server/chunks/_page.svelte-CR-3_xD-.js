import { j as ensure_array_like, c as attr_class, d as stringify, f as store_get, k as unsubscribe_stores } from './index2-CCZAVGyA.js';
import { i as isHydeStore } from './theme-BGWSFPhB.js';
...
function _page($$renderer) {
  ...
  $$renderer.push(`<div class="max-w-3xl mx-auto px-6 md:px-10 py-12"><div class="space-y-4"><!--[-->`);
  const each_array = ensure_array_like(menuItems);
  for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
    let item = each_array[$$index];
    $$renderer.push(`<button${attr_class(`w-full text-left bg-[#11141c] ${stringify(store_get($$store_subs ??= {}, "$isHydeStore", isHydeStore) ? "!bg-[#200505] border-red-900/40 hover:border-red-700/60" : "border-gray-800 hover:border-gray-600")} border rounded-xl px-10 py-8 transition-all duration-200 cursor-pointer group`)}><div class="flex items-center gap-6 m-3"><div${attr_class(`flex-shrink-0 w-12 h-12 rounded-lg ${stringify(store_get($$store_subs ??= {}, "$isHydeStore", isHydeStore) ? "bg-red-950/40" : "bg-gray-800/80")} flex items-center justify-center transition-colors`)}><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"${attr_class(`${stringify(store_get($$store_subs ??= {}, "$isHydeStore", isHydeStore) ? "text-red-400" : "text-blue-400")} transition-colors`)}><path${attr("d", item.icon)}></path></svg></div> <div class="flex-1 min-w-0"><div class="text-white font-semibold text-sm group-hover:text-gray-100">${escape_html(item.label)}</div> <div${attr_class(`${stringify(store_get($$store_subs ??= {}, "$isHydeStore", isHydeStore) ? "text-red-800" : "text-gray-500")} text-xs mt-0.5 transition-colors`)}>${escape_html(item.description)}</div></div> <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"${attr_class(`${stringify(store_get($$store_subs ??= {}, "$isHydeStore", isHydeStore) ? "text-red-700" : "text-gray-600")} group-hover:translate-x-1 transition-all`)}><path d="m9 18 6-6-6-6"></path></svg></div></button>`);
  }
...
//# sourceMappingURL=_page.svelte-CR-3_xD-.js.map
